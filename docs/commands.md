...
| `game:build-location-spatial`    | Build `data/enrichment/location-spatial.json` from export `locations.json` (SCANZ)                                | `--export=`, `--output=` — See [SCANZ-SETUP.md](SCANZ-SETUP.md)                                                                                                                                |
| `game:import-location-spatial`   | Import derived XYZ bundle into DB for a game version (SCANZ)                                                      | `<version>`, `--path=`, `--dry-run`                                                                                                                                                             |
| `game:repair-starmap-slugs`      | Reassign slugs using type suffixes (`stanton-solarsystem`) instead of `-2` (SCANZ)                                | `<version>`, `--dry-run`                                                                                                                                                                        |
